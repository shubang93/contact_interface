...
bool ContactInterfaceNode::initialize() {
    ...
    pose_sub = nh->subscribe<geometry_msgs::PoseStamped>(
        "mavros/local_position/pose", 10, &ContactInterfaceNode::pose_callback,
        this);
    mode_sub = nh->subscribe<std_msgs::Bool>(
        "has_control", 10, &ContactInterfaceNode::mode_callback, this);
    arm_sub = nh->subscribe<std_msgs::Bool>(
        "is_armed", 10, &ContactInterfaceNode::arm_callback, this);
    heading_sub = nh->subscribe<std_msgs::Float32>(
        "depth_estimation/point_cloud/heading_angle", 10,
        &ContactInterfaceNode::heading_callback, this);
    depth_sub = nh->subscribe<geometry_msgs::Vector3Stamped>(
        "depth_estimation/point_cloud/pose", 10,
        &ContactInterfaceNode::depth_callback, this);
    contact_command_sub = nh->subscribe<contact_interface::ContactCommand>(
        "controller/contact/command", 10,
        &ContactInterfaceNode::contact_command_callback, this);
    status_publisher_timer = nh->createTimer(
        ros::Duration(1. / status_pub_rate),
        &ContactInterfaceNode::status_publisher_timer_callback, this);

    // init publishers
    pose_command_pub =
        nh->advertise<geometry_msgs::PoseStamped>("pose_command", 10);
    contact_status_pub = nh->advertise<contact_interface::ContactStatus>(
        "controller/contact/status", 10);
    att_mode_pub = nh->advertise<core_px4_interface::AttMode>(
        "controller/commands/set_att_mode", 10);
    max_xy_vel_pub = nh->advertise<std_msgs::Float32>(
        "controller/commands/set_max_xy_speed", 10);
    return true;
}
...
bool ContactInterfaceNode::execute() {
    ...
    if (!is_offboard || !is_armed) {
        ...
        if (contact_status == ContactStatus::None &&
...
void ContactInterfaceNode::pose_callback(
    const geometry_msgs::PoseStamped::ConstPtr &msg) {
    current_pose = *msg;
}

void ContactInterfaceNode::contact_command_callback(
    const contact_interface::ContactCommand::ConstPtr &msg) {
    // return if it's not in offboard mode or if it's not armed
    if (!is_offboard || !is_armed) {
        contact_status = ContactStatus::None;
        task_status = TaskStatus::Aborted;
...
void ContactInterfaceNode::process_depth_reading(float depth, float angle) {
    ...
    if (dist <= stop_distance) {
        ...
        return;
    } else if (dist <= tilt_lock_distance &&
               approach_status == ApproachStatus::GettingClose) {
        change_status(ApproachStatus::LockingTilt, ContactStatus::Approaching,
                      TaskStatus::InProgress);
        return;
    }

    change_status(approach_status, ContactStatus::Approaching,
                  TaskStatus::InProgress);
...
void ContactInterfaceNode::heading_callback(
    const std_msgs::Float32::ConstPtr &msg) {
    // Do nothing for now
}

void ContactInterfaceNode::status_publisher_timer_callback(
...
void ContactInterfaceNode::publish_pose_command(
...
void ContactInterfaceNode::approach_getting_close() {
    ...
    auto curr_position = current_pose.pose.position;
    auto cmd_position = current_command.ContactPoint.position;

    double dx = curr_position.x - cmd_position.x;
    double dy = curr_position.y - cmd_position.y;
    double dz = curr_position.z - cmd_position.z;
...
void ContactInterfaceNode::contact() {
    ...
    if (ros::Time::now() - delay_started > ros::Duration(contact_duration)) {
        change_status(ApproachStatus::GettingClose, ContactStatus::Departing,
                      TaskStatus::InProgress);
    }
    ROS_WARN_THROTTLE(1, "%0.1f seconds in contact",
                      (ros::Time::now() - delay_started).toSec());
}
...
void ContactInterfaceNode::depart() {
    ...
    if (dist > dist_threshold * dist_threshold) {
        ...
        change_status(ApproachStatus::GettingClose, ContactStatus::None,
...
BaseNode *BaseNode::get() {
    ContactInterfaceNode *control_interface_node =
        new ContactInterfaceNode("ControlInterfaceNode");
